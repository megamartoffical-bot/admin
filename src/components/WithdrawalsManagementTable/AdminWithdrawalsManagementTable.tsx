...
export const AdminWithdrawalsManagementTable: React.FC<Props> = ({
  data,
  onDelete,
  refetch
}) => {
      ...
      <div className="overflow-x-auto my-6 bg-white shadow rounded-lg border border-gray-200">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              {['Amount', 'Method', 'Account', 'Date', 'Status', 'Actions'].map(
                header => (
                  <th
                    key={header}
                    scope="col"
      ...
      {selectedWithdrawal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-white/30 backdrop-blur-sm">
          <div className="bg-white rounded-lg  w-96 shadow-lg relative">
            {/* Close button */}
            <button
              className="absolute top-2 right-2 text-gray-100 hover:text-gray-200 font-bold text-2xl"
            ...
            {/* Withdrawal Details */}
            <WithdrawalsView withdrawal={selectedWithdrawal} refetch={refetch} setSelectedWithdrawal={setSelectedWithdrawal} />
          </div>
        </div>
      )}
    </>
