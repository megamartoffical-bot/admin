...
const VendorAddNewProductPage = () => {
  const [isImportModalOpen, setIsImportModalOpen] = useState(false);
  const [importedData, setImportedData] = useState<any>(null);
  const [formKey, setFormKey] = useState(0);

  const handleImport = useCallback((productData: any) => {
    console.log('Product data received in VendorAddNewProductPage:', productData);
  ...
  return (
    <div className="py-6 p-2 sm:p-4">
      ...
      <VendorAddProductForm key={formKey} importedData={importedData} />
...
export default VendorAddNewProductPage;
