...
export default function AddFaqPage() {
  ...
  const { register, handleSubmit, setValue, formState: { errors }, reset } = useForm<FaqFormData>();
    ...
    <div className="p-4 sm:p-6 space-y-6 lg:py-6">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div className="flex flex-col sm:flex-row sm:items-center sm:gap-4 min-w-0">
          <Link href="/faqs" className="inline-flex items-center text-base font-medium text-[#1A1A2E] hover:underline">
            <ArrowLeft className="w-5 h-5 mr-2" />
            Back to FAQs
          </Link>
          <span className="text-xl md:text-3xl font-semibold whitespace-nowrap">Add New FAQ</span>
        </div>
      ...
      <form className="grid grid-cols-1 xl:grid-cols-3 gap-6" onSubmit={handleSubmit(onSubmit)}>
        <Card className="xl:col-span-2">
          <CardHeader>
            ...
            <p className="text-sm text-muted-foreground">Create a new frequently asked question and answer.</p>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="question">Question*</Label>
              <Input id="question" {...register("question", { required: true })} placeholder="Enter the question" />
              {errors.question && <p className="text-xs text-red-500">Question is required</p>}
            </div>
            <div className="space-y-2">
              <Label htmlFor="answer">Answer*</Label>
              <Textarea id="answer" className="h-[138px]" rows={6} {...register("answer", { required: true })} placeholder="Enter the detailed answer" />
              {errors.answer && <p className="text-xs text-red-500">Answer is required</p>}
            </div>
            <div className="space-y-2">
              <Label htmlFor="category">Category*</Label>
              <Select onValueChange={(value: "global" | "shop") => setValue("category", value)}>
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <p className="text-sm text-muted-foreground">Control when and how this FAQ appears.</p>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div>
                <p className="text-xs text-[#979797]">Make this FAQ visible to users</p>
