...
export const ImageUploader = ({
  ...
  useEffect(() => {
    if (importedData) {
      setFeaturedPreview(importedData.images[0] || '');
      setGalleryPreviews(importedData.images.slice(1) || []);
    }
  }, [importedData]);
        ...
        <div className="flex justify-center">
          <div className="relative w-full h-64 border rounded-md overflow-hidden flex items-center justify-center bg-gray-50">
            {featuredPreview ? (
              ...
              />
            ) : EditableProcut?.featuredImg ? (
              <Image
                src={EditableProcut.featuredImg}
                alt="Preview"
                fill
                className="object-cover"
              />
