...
export default function VendorEditProductForm({ id }: { id: string }) {
  ...
  const { data: allShopData, isLoading: isShopDataLoading } =
    useGetAllShopsQuery();
  ...
  useEffect(() => {
    ...
    if (allShopData) {
      dispatch(setShops(allShopData));
    }
  }, [dispatch, tagsDatau, categoriesDatau, brandsu, allShopData]);
  ...
  // Filter shops for the current vendor
  const shopData = allShops?.filter((shop: any) => {
    const vendorIdToMatch = vendor?._id;
    const shopVendorId = shop.vendorId?._id || shop.vendorId;
    return shopVendorId === vendorIdToMatch;
  }) || [];
...
useEffect(() => {
  if (isReady) {
    ...
    const defaultValues = {
      ...
      brandAndCategories: {
        ...
        categories:
          product?.brandAndCategories?.categories?.map((cat: any) => cat?._id) || [],
      ...
      description: {
        ...
        status: product?.description?.status || "draft",
      },
      ...
      specifications: product?.specifications?.length as any > 0  ? product?.specifications: [{ key: "", value: "" }],

      variants: product?.variants?.length as any > 0 ? product?.variants : [],
    };
  ...
  if (shopData.length === 0 && !isShopDataLoading) {
    return (
      <div className="flex flex-col justify-center items-center h-64 gap-4">
        <p className="text-gray-500">You don't have any shops yet!</p>
        <p className="text-sm text-gray-400">Please create a shop before editing products.</p>
        <Button onClick={() => router.push("/vendor/create-shop")}>
